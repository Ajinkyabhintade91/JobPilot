...
export function useJobs(filters: JobFilterState) {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<Job[]> => {
      let q = supabase
        .from('jobs')
        .select('id,title,url,source,location,remote_type,match_score,status,tags,manual_apply_only,is_ghost_suspect,duplicate_of,posted_at,first_seen_at,description,companies(name)')
